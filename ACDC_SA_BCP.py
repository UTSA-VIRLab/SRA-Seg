...
parser.add_argument("--max_iterations", type=int, default=50000, help="maximum iteration number to self-train")
...
parser.add_argument("--labelnum", type=int, default=7, help="number of labeled patients")
...
# def generate_mask(img, hole_frac=2/3, kernel_size=3):
#     B, _, H, W = img.shape
...
def gaussian_kernel(kernel_size: int, sigma: float, device):
    ...
    return (kernel / kernel.sum()).unsqueeze(0).unsqueeze(0)  # shape (1,1,k,k)

def generate_mask(img, hole_frac=2/3, kernel_size=5, sigma=1.0):
    B, _, H, W = img.shape
    # 1) make the hard hole mask
    m = torch.ones((H, W), device=img.device, dtype=torch.float32)
    hole_h, hole_w = int(H * hole_frac), int(W * hole_frac)
    w = torch.randint(0, H - hole_h + 1, ())
    h = torch.randint(0, W - hole_w + 1, ())
    m[w:w+hole_h, h:h+hole_w] = 0

    # 2) build and apply Gaussian kernel
    gauss = gaussian_kernel(kernel_size, sigma, img.device)
    alpha = F.conv2d(m[None,None], gauss, padding=kernel_size//2).squeeze()

    # 3) build loss_mask the same way as before (dilated hole)
    inv = 1.0 - m
    eroded = F.max_pool2d(inv[None,None], kernel_size, stride=1, padding=kernel_size//2).squeeze()
    loss_mask = (1.0 - eroded).long().unsqueeze(0).expand(B, H, W)

    return m.long(), alpha, loss_mask, (w.item(), h.item())
