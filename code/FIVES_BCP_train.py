...
parser.add_argument('--pre_iterations', type=int, default=5000, help='maximum epoch number to train')
...
parser.add_argument('--s_map', type=int,  default=True, help='Using saliency map for cropping')

args = parser.parse_args()

# dice_loss = losses.DiceLoss(n_classes=2)
atvmf_dice_loss = losses.Adaptive_tvMF_DiceLoss(n_classes=2)
...
def compute_saliency_map(img):
    ...
    return torch.tensor(saliency_maps).cuda()


def find_salient_region(saliency_map, patch_x, patch_y, most_salient=True):
    img_x, img_y = saliency_map.shape
    best_saliency = -1 if most_salient else float('inf')
    top_left_x, top_left_y = 0, 0

    for i in range(img_x - patch_x):
        for j in range(img_y - patch_y):
            patch_saliency = saliency_map[i:i + patch_x, j:j + patch_y].sum()
            if most_salient:
                if patch_saliency > best_saliency:
                    best_saliency = patch_saliency
...
def generate_mask(img):
    ...
    # Determine adaptive patch size as a random proportion of image dimensions
    patch_x = int(img_x * np.random.uniform(min_ratio, max_ratio))
    patch_y = int(img_y * np.random.uniform(min_ratio, max_ratio))

    # Randomly select top-left corner of the patch within image bounds
...
# def generate_mask(img, weak_save_folder, strong_save_folder, use_saliency=False):
#     batch_size, channel, img_x, img_y = img.shape
#     loss_mask = torch.ones(batch_size, img_x, img_y).cuda()
#     mask = torch.ones(img_x, img_y).cuda()
...
#
#     if use_saliency:
#         saliency_maps = compute_saliency_map(img)
#         for i in range(batch_size):
#             if i % 2 == 0:  # Even index, find the weak (least salient) region
#                 w, h = find_salient_region(saliency_maps[i], patch_x, patch_y, most_salient=False)
#                 # save_image_with_highlight(img[i], w, h, patch_x, patch_y, weak_save_folder, f"weak_image_{i}.png")
#             else:  # Odd index, find the strong (most salient) region
#                 w, h = find_salient_region(saliency_maps[i], patch_x, patch_y, most_salient=True)
#                 # save_image_with_highlight(img[i], w, h, patch_x, patch_y, strong_save_folder, f"strong_image_{i}.png")
#
#             mask[w:w + patch_x, h:h + patch_y] = 0
#             loss_mask[i, w:w + patch_x, h:h + patch_y] = 0
#     else:
#         w = np.random.randint(0, img_x - patch_x)
#         h = np.random.randint(0, img_y - patch_y)
#         mask[w:w + patch_x, h:h + patch_y] = 0
#         loss_mask[:, w:w + patch_x, h:h + patch_y] = 0
#         # save_image_with_highlight(img, w, h, patch_x, patch_y, strong_save_folder, f"norm_bcp_image.png")
#
#     return mask.long(), loss_mask.long()


def save_image_with_highlight(img, w, h, patch_x, patch_y, save_folder, filename):
    if img.dim() == 4:
        img = img[0]
    ...
    cv2.rectangle(img_copy, (h, w), (h + patch_y, w + patch_x), (0, 0, 255), 2)
...
def mix_loss(output, img_l, patch_l, mask, l_weight=1.0, u_weight=0.5, unlab=False):
    ...
    patch_mask = 1 - mask
    loss_dice = atvmf_dice_loss(output_soft, img_l.unsqueeze(1), mask.unsqueeze(1)) * image_weight
    loss_dice += atvmf_dice_loss(output_soft, patch_l.unsqueeze(1), patch_mask.unsqueeze(1)) * patch_weight
...
def pre_train(args, snapshot_path):
    ...
    for _ in iterator:
        for _, sampled_batch in enumerate(trainloader):
            ...
            # Clamp the label values to be within the valid range [0, num_classes - 1]
            lab_a = torch.clamp(lab_a, 0, args.num_classes - 1)
            lab_b = torch.clamp(lab_b, 0, args.num_classes - 1)
