...
parser.add_argument('--saved_path', type=str, default='./model/BCP/best models/BCP with synthetic/')
...
BCP_model_path = FLAGS.saved_path + f'10%ACDCunet_best_model.pth'
...
def plot_kde(BCP_feature, BCP_pred, labels, specific_c, f_dim, pic_num):
    ...
    correct_cor_l, correct_cor_u = np.intersect1d(l_pred[0], l_lab[0]), np.intersect1d(u_pred[0],
                                                                                       u_lab[0]) + labeled_pixel
    # l_lab, u_lab = np.where(labels[:labeled_pixel,]==specific_c), np.where(labels[labeled_pixel:,]==specific_c)
    ...
    BCP_feature_l, BCP_feature_u = np.mean(BCP_feature[correct_cor_l[:pixel_num],], axis=1), np.mean(
        BCP_feature[correct_cor_u[:pixel_num],], axis=1)
    ...
    plt.savefig(
        f"KDE/ACDC/{f_dim}/labeled_{FLAGS.labelnum}/class_{specific_c}/kde_test_mean{pic_num}_{FLAGS.labelnum}_{specific_c}.png")
    print(
        f"Save to: KDE/ACDC/{f_dim}/labeled_{FLAGS.labelnum}/class_{specific_c}/kde_test_mean{pic_num}_{FLAGS.labelnum}_{specific_c}.png")
    plt.clf()


def Inference(FLAGS):
    os.environ['CUDA_VISIBLE_DEVICES'] = FLAGS.gpu
    BCP_Net = net_factory(net_type=FLAGS.model, in_chns=1, class_num=FLAGS.num_classes, mode="train")
    ...
    batch_sampler = TwoStreamBatchSampler(labeled_idx, unlabeled_idxs, FLAGS.batch_size,
